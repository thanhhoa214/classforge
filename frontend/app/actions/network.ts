...
export async function getDummyNetworkData(
  type: NetworkType
): Promise<NetworkData> {
...
export async function getNetworkData(type: NetworkType): Promise<NetworkData> {
  try {
    ...
    if (networks.length === 0) {
      console.log(
        `No network data found for type ${type}. Returning dummy data.`
      );
    ...
    if (students.length === 0 && networks.length > 0) {
      console.log(
        `Network links exist for type ${type}, but no corresponding students found. Returning dummy data.`
      );
      return getDummyNetworkData(type);
    }

    // Create a map for quick lookup
    const studentMap = new Map<string, Pick<Student, "id" | "name">>(
    ...
    if (finalNodes.length === 0 && edges.length === 0) {
      console.log(
        `Processed data for type ${type} resulted in empty nodes/edges. Returning dummy data.`
      );
      return getDummyNetworkData(type);
    }
